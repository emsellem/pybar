...
def resample_data(Xin, Yin, Zin, newextent=None, newstep=None, fill_value=np.nan, method='linear', verbose=False) :
    ...
    if not test : return None, None, None, 0
...
class mybar(object) :
    ...
    def get_PatternSpeed(self, step_factor=1.0, fill_value=None, method=None) :
    ...
    def get_PatternSpeed_2(self, step_factor=1.0, fill_value=None, method=None) :
        if fill_value == None : fill_value = self.fill_value
        if method == None : method = self.method
